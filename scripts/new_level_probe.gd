...
func setup() -> void:
	...
	_check(mesh_count == 0,
			"资产约束：场景未混入MeshInstance3D成品模型，模型全部由CSG白盒构成")
	...
	_check(_count_group(level, "team_waiting_room") == 4 \
			and _count_group(level, "waiting_room_cart_bay") == 12 \
			and _spawns_inside_waiting_rooms(gameplay["player_spawns"]),
			"等待室：四队入口外均有购物车缓冲区，8个玩家席位全部出生在等待室")
	...
	_check(_m.all_items.size() == expected_stock + 6 \
			and _m.get_tree().get_nodes_in_group("region_kids").size() == 6 \
			and _m.get_tree().get_nodes_in_group("region_grannies").size() == 8,
			"实际开局：%d件货架库存＋6件乱蹦生鲜，另有6熊孩子/8挡路大妈（实际商品%d/孩子%d/大妈%d）" % [
				expected_stock, _m.all_items.size(),
				_m.get_tree().get_nodes_in_group("region_kids").size(),
				_m.get_tree().get_nodes_in_group("region_grannies").size()])
	_check(_tiered_stock_ready(expected_stock),
			"三级铺货：各专区低/中/高档按6:3:1循环分布，货架商品统一展示为车内尺寸2倍")
	...
	_check(checkouts_ready, "实际开局：八条通道均生成向上开启的单车入口隔离门、扫码区和防倒灌出口门")
	var open_by_side := {}
	...
	_check(audit != null and audit.projection == Camera3D.PROJECTION_ORTHOGONAL \
			and audit.size >= 50.0 and audit.position.y >= 55.0,
			"排布审查：正上方正交相机覆盖完整卖场")
...
func _tiered_stock_ready(expected_total: int) -> bool:
	...
	for it in _m.all_items:
		...
		if not is_instance_valid(it) or it.state != Item.ItemState.SHELVED \
				...
				or it.label.billboard != BaseMaterial3D.BILLBOARD_DISABLED:
			return false
	...
	for zone in Catalog.SHOPPING_ZONES + [Catalog.ZONE_PREMIUM]:
		...
		var expected := Catalog.PREMIUM_STOCK if zone == Catalog.ZONE_PREMIUM \
				else Catalog.SHELF_STOCK_PER_ZONE
		if total != expected:
			return false
		var cycles := int(expected / 10)
		if int(counts.get("%s:1" % zone, 0)) != cycles * 6 \
				or int(counts.get("%s:2" % zone, 0)) != cycles * 3 \
				or int(counts.get("%s:3" % zone, 0)) != cycles:
			return false
	return true
...
func _spawns_inside_waiting_rooms(spawns: Array) -> bool:
	if spawns.size() != 8:
		return false
	for value in spawns:
		var p := value as Vector3
		var in_x := (p.x >= -26.0 and p.x <= -18.0) \
				or (p.x >= 18.0 and p.x <= 26.0)
		var in_z := (p.z >= 22.5 and p.z <= 31.5) \
				or (p.z >= -31.5 and p.z <= -22.5)
...
func _verify_central_opening() -> bool:
	...
	for it in _m.central_locked_items:
		if not is_instance_valid(it):
			continue
		if it.item_id != _m.central_feature_id or it.event_locked or not it.visible:
			return false
		selected_alive += 1
	if selected_alive <= 0:
		return false
...
func _verify_region_effects(bounds: Dictionary) -> bool:
	...
	var cold_ok := p.cold_meter >= 0.99 and p.frozen_time >= 7.9
	...
	var beauty_ok := p.obscure_time <= 0.0 and fog_volume != null \
			and fog_material != null and fog_material.density >= 0.4 \
			and fog_material.edge_fade >= 2.0
